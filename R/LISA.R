...
tmap_LISA <- function(shp, var) {
  LISA_palette <- c("Low-Low" = "dodgerblue3", 
                    "Low-High" = "dodgerblue", 
                    "High-Low" = "firebrick1", 
  ...
  t <- tm_shape(shp) + 
    tm_polygons(col = var, palette = LISA_palette, border.alpha = 0.2, border.col = "white")
  
  t
  
}

LISA_list <- c("MHLTH_LISA", "DEPRESSION_LISA", "SLEEP_LISA", "NDVI_MEAN_LISA") %>% 
  map(~tmap_LISA(shp = zcta_main2 %>% st_simplify(dTolerance = 50), var = .x))
