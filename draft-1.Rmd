...
```{r extent, fig.width = 4.5, fig.cap = "Extent of ZIP codes (dark gray) within Western NC (light gray)"}
knitr::include_graphics("fig/brownExtent.png", dpi = 600)
```
...
```{r gs-extent, fig.width = 4.5, fig.cap = "Extent of combined PAD-US and ParkServe greenspace areas (dark gray)"}
knitr::include_graphics("fig/brownGSExtent.png", dpi = 600)
```
...
knitr::kable(moran,
             ...
             caption = "Calculated statistics for Moran’s I across predictor and response variables")
...
basic_list %>% map_dfr(regclass::VIF) %>% t() %>% `row.names<-`(c("Mean NDVI", "Greenspace Area", "Percent White", "Income", "Amenities", "Trails")) %>% `colnames<-`(c("MHLTH", "DEPRESSION", "SLEEP")) %>% 
  knitr::kable(digits = 4, caption = "Variance Inflation Factors for variables")
...
knitr::kable(l, caption = "Results from Robust Lagrange Multiplier Test. Spatial lag regression was used in this analysis")
```
...
```{r lisa, fig.width=4.5, fig.cap = "Results from LISA highlighting general trend from hot (high-high) to cold(low-low) clustering of each variable. Note that not all locations have a p-value < 0.05 and that this figure is meant as a visual aid"}
...
model_list %>%
  ...
  texreg::texreg(
    ...
    float.pos = "h", caption = "Spatial lag regression coefficients for each mental health outcome"
  )
```
